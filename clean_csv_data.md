...
### 新增功能
- **智能 lot_id 提取：** 从文件夹名称中智能提取 product_name 和 lot_id
- **双重命名支持：** 支持提取产品名称和批次ID分别用于不同目的

### 提取规则
- **包含 `_` 符号的文件夹名：**
  - `product_name`: `_` 之前的部分
  - `lot_id`: `_` 后面到 `@` 之间的部分
  - 示例：`NCETSG7120BAA_FA54-5342@203` → 产品名=`NCETSG7120BAA`, 批次ID=`FA54-5342`

- **不包含 `_` 符号的文件夹名：**
  - `product_name` 和 `lot_id` 都使用原始文件夹名称
  - 示例：`PLAINNAME` → 产品名=`PLAINNAME`, 批次ID=`PLAINNAME`

### 应用场景
1. **单层结构：** 直接使用文件夹名称应用提取规则
2. **双层结构：** 使用各个子文件夹名称分别应用提取规则
