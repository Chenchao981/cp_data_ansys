...
Cockpit 左侧提供批次、片号和参数筛选器，均默认全选，也可保留任意 1 个或多个选项。标准 CSV 输出目录可直接输入，也可点击文件夹按钮选择；独立启动时默认 Windows 用户桌面，GUI 打开时沿用当前清洗输出目录。片号按 `Lot_ID + Wafer_ID` 组合键显示为“批次 / W片号”，避免不同批次存在相同片号时混淆。“良品 Bin 范围”统一定义 Good Die，可输入 `1`、`1,3` 或 `1-3,7`；该口径同时用于 Cockpit 的良率、失效判断、综合 Bin Mapping 和“仅 Good Die”参数样本。首次打开只加载筛选项，不生成图表；点击“绘制图形”后才按当前范围生成分析页。筛选只改变当前分析视图，不修改标准 CSV。
...
- 自定义良品 Bin 范围（例如 `1,3,5-7`）与 Good Die 参数样本筛选
- 单级图表导航，所有分析页一目可见
