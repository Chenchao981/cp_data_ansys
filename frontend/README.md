...
参考 `F:\vdmos\VDMOS_Tool_v5.6.html` 的数据管理交互，Cockpit 左侧提供三个独立操作：`保存数据` 把本次使用的 cleaned、yield 和全部 spec CSV 保存为 ZIP；`加载数据` 打开文件选择入口，选中 ZIP 后立即校验、加载并恢复全部图表；`重置` 清空本次待加载文件选择并重新打开选择入口。压缩包保留版本清单、原始文件名、大小和 SHA-256；多 Lot 独立规格继续按 Lot 隔离，旧版 `.cpcockpit` 文件仍可载入。只借鉴交互，不引入 VDMOS 数据解析或用户界面名称。
