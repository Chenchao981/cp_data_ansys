...
标准 CSV 输出目录可直接输入或点击文件夹按钮选择，独立启动时默认用户桌面。片号按“批次 / W片号”显示，避免跨批次同号 Wafer 混淆。“良品 Die 筛选”中的良品 Bin 范围（例如 `1,3,5-7`）统一决定 Cockpit 的良率、失效和 Mapping 判定；参数样本可选择“全部 Die”或“仅 Good Die”。图表通过左侧单级菜单切换。首次打开不自动绘图；选择完成后点击“绘制图形”。
