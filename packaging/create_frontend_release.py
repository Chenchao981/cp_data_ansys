...
片号按“批次 / W片号”显示，避免跨批次同号 Wafer 混淆。参数样本范围还可选择“全部 Die”或“仅 Good Die（Bin = Pass Bin）”；Good Die 只影响参数分析和 cleaned 预览，良率与失效图仍使用全部 Die。图表通过左侧分组折叠菜单切换。首次打开不自动绘图；选择完成后点击“绘制图形”。
