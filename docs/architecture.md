...
flowchart LR
    ...
    C2["Lion 管芯数 Excel"] --> L2["lion_die_count_processor.py"]
...
GUI 支持 Excel 文件夹、单个/多个 ZIP 和只包含 ZIP 的文件夹。准备完成后由 `lion_batch_processor` 发现批次，并按工作簿内容严格分派：成熟 V1 继续使用 `LionExcelReader`，根据精确的 `summary_information` / `dut_data` Sheet、八个固定过程字段和三行规格结构识别，不依赖固定测试参数数量。V1 将 `TEST_NUM` 右侧列按源顺序动态解析；同一 Lot 内按首次出现顺序合并参数并集，某片未执行的测试项在 cleaned 中留空，同一 Lot 内同名参数的单位、下限、上限或测试条件冲突时 fail closed。不同 Lot 可使用不同版本的同名参数规格，不能把一个 Lot 的规格当成全局规格。格式 2 使用独立 `LionV2Reader` / `LionV2Adapter`，要求 OLE `.xls`、精确的 `Summary information` / `Statistics Information` / `DUT_DATA` Sheet 集和已批准的有序参数 schema。当前格式 2 注册了原 15 参数 schema 与产品 F0122A1 的 14 参数 schema；并未放宽为任意参数数量。未知、歧义或同一次运行混合 V1/V2 会 fail closed。两种格式均合并输出 cleaned/yield，并保留每行原始 `Lot_ID`；多 Lot 运行在同一目录按 Lot 输出原有横向 spec，禁止采用第一片或第一 Lot 覆盖。CP Cockpit 检测到多份 spec 时按用户选择的 Lot 同步隔离 cleaned、yield 和规格。

`lion-管芯数` 为独立的 Wafer 级汇总流程，由 `lion/lion_die_count_processor.py` 递归发现 `.xlsx` 并按内容严格分派。格式 1 要求单 `Sheet`、`WAFER PROBE FAIL COUNTER REPORT` 标识、唯一 `DEVICE=` / `LOT#=` 及 `WAFER# / PASS / DIE`；格式 2（LCD235）要求单 `Sheet1`、立昂微 MOSFET 报告标识和已批准的完整 34 列有序表头，并校验汇总总片数、CP合格管芯总数和合格管芯总数。两种格式均输出五列 Excel；LCD235 的 `PASS = CP合格管芯数`，`Good Die = CP合格管芯数 - QAD补点数`。该流程不经过 `CPLot` 和标准 CSV/图表层，因此不会影响 Lion CP V1/V2 数据契约。
