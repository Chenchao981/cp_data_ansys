...
5. 先清洗数据，再点击当前公司页面中的 `CP Cockpit` 进行交互分析。批次、片号、参数默认全选；取消对应“全选”后可选择 1 个或多个选项。参数分析需要只看良品时，选择“仅 Good Die（Bin = Pass Bin）”；该范围不改变良率和失效类图表。完成后点击“绘制图形”，再通过左侧分组菜单切换页面。首次打开和筛选变更时不会自动重算图表。左侧“数据管理”可 `保存数据` 为 ZIP，或通过 `加载数据` 浏览并恢复分析数据；这些操作不会删除磁盘上的原始 CSV。
