...
华虹 ZIP/7z 内应包含 DCP/TXT；JT、Lion、国宇 FRD ZIP 内应包含 Excel。可以选择单个压缩文件、一次多选多个，或选择直接存放这些压缩文件的文件夹；加密、损坏、缺少对应数据或包含不安全路径的压缩文件会被拒绝并在 GUI 日志中说明原因。Lion 支持动态参数 V1 与已验收的三 Sheet 格式 2：V1 会按每片表头自动读取 `TEST_NUM` 之后的测试项，同批次允许参数增减并生成并集规格；同名参数规格冲突仍会停止。格式 2 当前接受原 15 参数 schema 和 F0122A1 14 参数 schema，其他未知 schema 会停止，同一次运行也不得混合 V1/V2。格式 2 多 Lot 可能生成多份按 Lot 命名的 spec，进入 CP Cockpit 后先选择“参数分析 Lot”。国宇 ZIP 应保留批次/EDS 层级；程序会识别批次 ZIP 和产品多批次 ZIP。
