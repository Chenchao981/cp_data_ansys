...
```powershell
packaging\release_frontend\start.bat

# 仅检查 Python、依赖和 app.pyz，不打开 GUI
packaging\release_frontend\start.bat --check

# Git Bash
packaging/release_frontend/start.sh
packaging/release_frontend/start.sh --check
```
...
1. 选择公司。
2. 输入和输出路径默认显示当前 Windows 用户桌面，用户可直接输入路径或点击浏览按钮修改。
3. 每家公司页面都保留“选择文件夹”和“选择ZIP”两个入口；ZIP 可一次多选，也可通过“选择文件夹”选择只包含 ZIP 的目录。输出路径始终选择输出父目录。
4. 先清洗数据，再点击当前公司页面中的 `CP Cockpit` 进行交互分析。
...
`packaging/release_frontend` 是唯一主要发布目录。入口为 `gui.multi_company_main:main`，产物写入 `packaging/release_frontend/app.pyz`，并同时包含 CP Cockpit 启动文件、发布文档和依赖安装脚本。打包脚本会排除原始数据、输出结果、日志、测试、缓存和 Git 文件。
