...
数据管理交互参考 `F:\vdmos\VDMOS_Tool_v5.6.html` 的三个独立操作，但保持 CP 数据契约不变：`保存数据` 下载上述 ZIP；`加载数据` 打开新的文件选择入口，校验和预览后重新计算统计并渲染图表；`重置` 清空当前会话中的数据、图表缓存和筛选控件，不删除任何磁盘文件。CP Cockpit 不引入 VDMOS 的 JSON 项目格式、原始文件解析、规格常量或用户界面名称。
