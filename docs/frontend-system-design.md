...
左侧分析表单提供批次、片号、参数筛选器。首次加载均为全选；用户可保留 1 个或多个选项。标准 CSV 输出目录可直接输入，或通过本机原生文件夹选择器选取；独立启动时默认 Windows 已知桌面，GUI 启动时使用当前清洗输出目录。片号选择项使用 `Lot_ID + Wafer_ID` 组合键，显示为“批次 / W片号”。“良品 Bin 范围”是 Good Die 的唯一口径，支持 `1`、`1,3`、`1-3,7` 等表达式；其同时决定 Cockpit 显示的良品数、良率、失效 Pareto、综合 Bin Mapping、区域/失效分析以及“仅 Good Die”的参数样本。页面先保存筛选草稿，首次打开不执行 Plotly 图表函数；只有点击“绘制图形”时才记录筛选快照。筛选草稿再次变化时停止展示旧图并要求重新点击，避免每次勾选都触发重计算。源 CSV 与测试值保持不变。

图表页采用单级侧边栏菜单，所有分析页均在同一层显示，一次只渲染当前选中的页面，避免旧的横向多页签在每次交互时同时计算全部 Plotly 图表。当前 Cockpit 压缩包以紧凑状态条显示，cleaned/yield/spec 文件清单放在侧边栏底部，不遮挡筛选和绘图操作。
...
- 单级图表导航：所有分析页直接可见
- 良率和 Bin
- 参数 BoxPlot（支持自定义 Good Die Bin 范围）
- 参数 Wafer 散点图（支持自定义 Good Die Bin 范围）
