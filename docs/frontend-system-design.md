...
左侧分析表单提供批次、片号、参数三个全局多选器。首次加载均为全选；用户可保留 1 个或多个选项。片号选择项使用 `Lot_ID + Wafer_ID` 组合键，显示为“批次 / W片号”。页面先保存筛选草稿，首次打开不执行 Plotly 图表函数；只有点击“绘制图形”时才记录筛选快照，把对应 cleaned/yield 视图传入全部图表和统计函数。筛选草稿再次变化时停止展示旧图并要求重新点击，避免每次勾选都触发重计算。源 CSV 与测试值保持不变。参数选择控制参数 BoxPlot、散点、Mapping 可选项、区域参数、Wafer Summary 和 Cpk 表。
