...
2. 在左侧“数据管理”中点击 `加载数据`。
3. 点击文件选择框的浏览按钮，手动选择此前保存的 ZIP 文件。
4. 先核对压缩包文件清单、数据行数、Lot/Wafer 数量和 cleaned 前 5 行预览。
5. 确认无误后点击 `加载并展示图表`，恢复全部交互图表，不需要再次清洗原始数据。多 Lot 文件仍会要求选择参数分析 Lot，避免误用其他 Lot 的规格。

### 重置

点击左侧“数据管理”中的 `重置`，会清空当前加载的数据、图表和筛选状态，但不会删除磁盘上的原始 CSV 或已经保存的 ZIP。重置后可重新加载当前输出目录，或点击 `加载数据` 选择保存文件。
