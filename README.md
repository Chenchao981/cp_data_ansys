...
四家公司 GUI 统一使用一个“选择数据源”入口：在同一窗口中可选择一个原始数据目录，或按 `Ctrl` / `Shift` 多选 ZIP 文件。GUI 只负责返回路径，后端自动判断目录或 ZIP 并调用原有独立处理流程。ZIP 仅在后台临时目录中安全解压，处理完成后自动清理。输出路径选择父目录，最终仍按原流程创建“首个真实批次号_YYYYMMDD_HHMMSS”文件夹。
