...
| :----------- | :------------------------------------------------------------------- | :--------------- | :---------------- | :----------------- | :------------------------------------------------------------------- |
| `Product_Name` | 产品名称                                                           | String           | "NCETSG7120BAA"   | "NCETSG7120BAA"    | 从文件夹名称中"_"前面部分提取，例如NCETSG7120BAA_FA54-5339@203        |
...
1.  **`Product_Name`**: 产品名称，从文件夹名称中"_"前面部分提取，例如从"NCETSG7120BAA_FA54-5339@203"中提取"NCETSG7120BAA"。
2.  **`Lot_ID`**: 直接从该晶圆数据中获取。
3.  **`Wafer_ID`**: 当前分组的 `Wafer_ID`。
4.  **`Total`**: 该 `Wafer_ID` 在输入数据中的总行数（即总Die数）。
5.  **`Pass`**: 该 `Wafer_ID` 数据中，`Bin` 列值为 `1` 的行数。
6.  **`Yield`**: `(Pass / Total) * 100`。结果需格式化为带两位小数的百分比字符串 (如 "98.45%")。如果 `Total` 为0，`Yield` 应为 "0.00%"。
7.  **`Bin3`**: 该 `Wafer_ID` 数据中，`Bin` 列值为 `3` 的行数。
8.  **`Bin4`**: 该 `Wafer_ID` 数据中，`Bin` 列值为 `4` 的行数。
9.  **`Bin6`**: 该 `Wafer_ID` 数据中，`Bin` 列值为 `6` 的行数。
10. **`Bin7`**: 该 `Wafer_ID` 数据中，`Bin` 列值为 `7` 的行数。
11. **`Bin8`**: 该 `Wafer_ID` 数据中，`Bin` 列值为 `8` 的行数。
12. **`Bin9`**: 该 `Wafer_ID` 数据中，`Bin` 列值为 `9` 的行数。

### 5.2. "ALL" 行（批次汇总）数据计算

1.  **`Product_Name`**: 与单个晶圆行相同的产品名称。
2.  **`Lot_ID`**: 固定字符串 "ALL"。
3.  **`Wafer_ID`**: 固定字符串 "ALL"。
4.  **`Total`**: 所有单个晶圆 `Total` 值的总和。
5.  **`Pass`**: 所有单个晶圆 `Pass` 值的总和。
6.  **`Yield`**: 计算所有单个晶圆 `Yield` 值（数值形式，非百分比字符串）的算术平均值。然后将此平均值格式化为带两位小数的百分比字符串。如果所有晶圆的Total都为0，则批次Yield为 "0.00%"。
7.  **`Bin3`**: 所有单个晶圆 `Bin3` 计数的总和。
8.  **`Bin4`**: 所有单个晶圆 `Bin4` 计数的总和。
9.  **`Bin6`**: 所有单个晶圆 `Bin6` 计数的总和。
10. **`Bin7`**: 所有单个晶圆 `Bin7` 计数的总和。
11. **`Bin8`**: 所有单个晶圆 `Bin8` 计数的总和。
12. **`Bin9`**: 所有单个晶圆 `Bin9` 计数的总和。
...
```csv
Product_Name,Lot_ID,Wafer_ID,Yield,Total,Pass,Bin3,Bin4,Bin6,Bin7,Bin8,Bin9
NCETSG7120BAA,FA54-5339,1,98.45%,193,190,0,1,2,0,0,0
NCETSG7120BAA,FA54-5339,2,98.45%,193,190,0,1,2,0,0,0
NCETSG7120BAA,FA54-5339,3,97.41%,193,188,1,2,2,0,0,0
...
NCETSG7120BAA,ALL,ALL,98.18%,4825,4737,10,19,51,3,2,3
``` 
...
这个公式展示了上下标：$x^{2+2}$ 和 $x_{12}$。

分数形式：$\frac{a}{b} + \frac{c}{d}$。